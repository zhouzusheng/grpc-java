 ...
 */
public class WebSocketUpgradeHandler implements AsyncHandler<NettyWebSocket>, AsyncWebSocketSender {

  private final List<WebSocketListener> listeners;
  private NettyWebSocket webSocket;
  private List<WebSocketFrame> pendingFrames;
  ...
  private synchronized void processPendingFrames() {
    try{
      if (pendingFrames != null) {
        for (WebSocketFrame frame : pendingFrames) {
          webSocket.sendWebSocketFrame(frame);
        }
      }
    } finally {
      releasePendingFrames();
  ...
  private synchronized void releasePendingFrames() {
    if (pendingFrames != null) {
      for (WebSocketFrame frame : pendingFrames) {
        frame.release();
      }
  ...
  private synchronized void addPendingFrame(WebSocketFrame frame){
    if(webSocket != null) {
      webSocket.sendWebSocketFrame(frame);
      return;
    }
    if (pendingFrames == null) {
      pendingFrames = new ArrayList<>(1);
    }
    frame.retain();
